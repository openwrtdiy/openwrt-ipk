...
qosdef_monitor_get_ip_handle() { # <family> <chain> <ip>
	echo $(nft -a list chain $1 pppoe-qos-monitor $2 2>/dev/null | grep $3 | awk '{print $11}')
}
...
qosdef_init_monitor() {
	...
	nft add table $NFT_QOS_INET_FAMILY pppoe-qos-monitor
	nft add chain $NFT_QOS_INET_FAMILY pppoe-qos-monitor upload { type filter hook $hook_ul priority 0\; }
	nft add chain $NFT_QOS_INET_FAMILY pppoe-qos-monitor download { type filter hook $hook_dl priority 0\; }
}
