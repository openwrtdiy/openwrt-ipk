...
qosdef_flush_mac() {
	if [ -n "$NFT_QOS_HAS_BRIDGE" ]; then
		qosdef_flush_table bridge nft-qos-unofficial-mac
	else
		qosdef_flush_table "$NFT_QOS_INET_FAMILY" nft-qos-unofficial-mac
	fi
}
...
qosdef_init_mac() {
	...
	[ "$2" = 0 ] || {
		logger -t nft-qos-unofficial-mac "validation failed"
		return 1
	}
	...
	qosdef_appendx "table $table_name nft-qos-unofficial-mac {\n"
