...
qosdef_validate_priority() {
	uci_load_validate nft-qos-unofficial default "$1" "$2" \
...
qosdef_remove_priority() {
	qosdef_remove_table netdev nft-qos-unofficial-priority
}

# init traffic priority
qosdef_init_priority() {
	[ "$2" = 0 ] || {
		logger -t nft-qos-unofficial-priority "validation failed"
		return 1
	}
	...
	[ -n "$ifname" ] || {
		logger -t nft-qos-unofficial-priority "unable to get ifname for $priority_netdev"
		return 1
	}

	qosdef_appendx "table netdev nft-qos-unofficial-priority {\n"
